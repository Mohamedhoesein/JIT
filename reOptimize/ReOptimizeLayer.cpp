...
PointerType *ReOptimizeLayer::getInt8PtrTy(LLVMContext &context) {
    return IntegerType::get(context, 8)->getPointerTo();
}
...
void ReOptimizeLayer::createReoptimizeCall(Module &M, Instruction &IP,
    ...
    if (!DispatchCtx)
        DispatchCtx = new GlobalVariable(M, ReOptimizeLayer::getInt8PtrTy(M.getContext()),
                                         false, GlobalValue::ExternalLinkage,
                                         nullptr, "__orc_rt_jit_dispatch_ctx");
    GlobalVariable *ReoptimizeTag =
            M.getGlobalVariable("__orc_rt_reoptimize_tag");
    if (!ReoptimizeTag)
        ReoptimizeTag = new GlobalVariable(M, ReOptimizeLayer::getInt8PtrTy(M.getContext()),
                                           false, GlobalValue::ExternalLinkage,
                                           nullptr, "__orc_rt_reoptimize_tag");
    Function *DispatchFunc = M.getFunction("__orc_rt_jit_dispatch");
    if (!DispatchFunc) {
        std::vector<Type *> Args = {ReOptimizeLayer::getInt8PtrTy(M.getContext()),
                                    ReOptimizeLayer::getInt8PtrTy(M.getContext()),
                                    ReOptimizeLayer::getInt8PtrTy(M.getContext()),
