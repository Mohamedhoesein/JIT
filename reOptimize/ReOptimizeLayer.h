...
namespace llvm {
    namespace orc {
        ...
        public:
            ...
            static const uint64_t CallCountThreshold = 2;
            ...
            ReOptimizeLayer(ExecutionSession &ES, IRLayer &BaseLayer,
                            std::unique_ptr<RedirectableSymbolManager> RM)
                    : IRLayer(ES, BaseLayer.getManglingOptions()), ES(ES),
                      BaseLayer(BaseLayer), RSManager(std::move(RM)), ReOptFunc(identity),
        ...
        private:
            static PointerType *getInt8PtrTy(LLVMContext &context);
